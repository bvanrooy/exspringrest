...
public class AppController {
	...
	public String addPerson(@Valid Person person, BindingResult result){
		logger.info("POST /addPerson {}", person != null ? person.toString() : "null");
		try {
			if (result.hasErrors()) {
				logger.info("POST /addPerson result {}", result.getAllErrors().toString());
				return "addPerson";
			} 
			logger.info("POST /addPerson calling trainingservice");
